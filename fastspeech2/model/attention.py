...
class MultiHeadAttention(nn.Module):
    def __init__(self, args, n_head):
        super(MultiHeadAttention, self).__init__()
        self.args   = args
        self.logger = args.logger

        # 음소 임베딩 크기는 head의 개수로 나누어 떨어져야만 함 (concat 때문)
        if args.phoneme_embedding_dimension % n_head != 0:
            self.logger.error(f"phoeneme_embedding_size({args.phoneme_embedding_dimension}) must be divided by number of heads({n_head})")
            raise ValueError("Phoneme embedding dimension must be divisible by the number of attention heads.")
         

        self.h_dim = args.phoneme_embedding_dimension // n_head
